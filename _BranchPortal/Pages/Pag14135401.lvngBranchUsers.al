    ...
    actions
    {
        area(Processing)
        {
            ...
            action(PerformanceMapping)
            {
                ...
                PromotedIsBig = true;
                PromotedCategory = Process;
                RunObject = page lvngBranchPerfMappingPart;
                RunPageLink = "User ID" = field("User ID");
                RunPageView = sorting(Sequence);
                ApplicationArea = All;
            }
            action(GLAccounts)
            {
                Caption = 'G/L Report Accounts';
                Image = Accounts;
                Promoted = true;
                PromotedCategory = Process;
                PromotedIsBig = true;
                RunObject = page lvngBranchUserGLAccounts;
                RunPageLink = "User ID" = field("User ID");
                ApplicationArea = All;
            }
            action(LoanLevelMapping)
            {
                Caption = 'Loan Funding Mapping';
                Image = MapDimensions;
                Promoted = true;
                PromotedCategory = Process;
                PromotedIsBig = true;
                RunObject = page lvngLoanLevelReportMappingPart;
                RunPageLink = "User ID" = field("User ID");
                RunPageView = sorting(Sequence);
                ApplicationArea = All;
            }
