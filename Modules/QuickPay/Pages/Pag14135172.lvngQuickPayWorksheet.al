page 14135172 lvngQuickPayWorksheet
{
    PageType = List;
    ApplicationArea = All;
    UsageCategory = Administration;
    SourceTable = lvngQuickPayBuffer;
    ...
    layout
    {
        area(Content)
        {
            group(Main)
            {
                ...
                group(Filters)
                {
                    ...
                    field(QuickPayPresetCode; QuickPayPresetCode) { ApplicationArea = All; Caption = 'Filter Preset'; TableRelation = lvngQuickPayFilterPreset.Code; }
                }
                group(Payment)
                {
                    Caption = 'Payment';
        ...
        area(FactBoxes)
        {
            ...
            part(Documents; lvngDocumentListFactbox) { ApplicationArea = All; Caption = 'Documents'; }
        }
    }
    ...
    var
        ...
        Buffer: Record lvngQuickPayBuffer temporary;
    ...
    var
        ...
        QuickPayPreset: Record lvngQuickPayFilterPreset;
        TempVendorLedgerEntry: Record "Vendor Ledger Entry" temporary;
        QuickPayData: Query lvngQuickPayData;
        FilterText: Text;
    begin
        Rec.Reset();
        ...
        if QuickPayPreset.Get(QuickPayPresetCode) then
            if QuickPayPreset.ApplyVendorLedgerEntries(TempVendorLedgerEntry) then begin
                ...
                FilterText := TempVendorLedgerEntry.GetFilter(lvngEntryDate);
                if FilterText <> '' then
    ...
    begin
        ...
        case PurchasesPayablesSetup.lvngQuickPayBatchSelection of
            PurchasesPayablesSetup.lvngQuickPayBatchSelection::"One Batch":
                begin
                    PurchasesPayablesSetup.TestField(lvngQuickPayDefaultBatch);
                    BatchName := PurchasesPayablesSetup.lvngQuickPayDefaultBatch;
                end;
            PurchasesPayablesSetup.lvngQuickPayBatchSelection::"Defined in User Setup":
                begin
                    UserSetup.Get(UserId());
                    UserSetup.TestField(lvngDefaultPaymentJournalBatch);
                    BatchName := UserSetup.lvngDefaultPaymentJournalBatch;
                end;
            PurchasesPayablesSetup.lvngQuickPayBatchSelection::Selectable:
                begin
                    GenJnlBatch.Reset();
        ...
        if Confirm(ConfirmJournalCreateQst, true) then begin
            ...
            if GroupPerVendor then begin
                ...
                repeat
                    ...
                    GenJnlLine.Description := GenJnlLine.lvngSourceName;
                ...
                repeat
                    ...
                    GenJnlLine.lvngDocumentGuid := Rec."Document GUID";
                    GenJnlLine."Source Code" := GenJnlTemplate."Source Code";
                    GenJnlLine.Validate("Bank Payment Type", BankPaymentType);
                    GenJnlLine.lvngLoanNo := Rec."Loan No.";
                    GenJnlLine.Insert(true);
                until Rec.Next() = 0;
            end;
            Commit();
