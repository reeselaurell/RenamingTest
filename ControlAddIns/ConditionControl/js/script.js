...
function AppendCondition() {
    ...
    clone.data('left', '');
    clone.data('right', '');
    clone.find('label span').text(condition_count);
    condition_host.append(clone);
    clone.find('button').click(function () {
        $(this).closest('.condition-control').remove();
        condition_count--;
        var idx = 1;
        $('.condition-control label span').each(function () {
            $(this).text(idx++);
        });
        Microsoft.Dynamics.NAV.InvokeExtensibilityMethod('ItemRemoved', []);
    });
    InitControls(clone);
    clone.find('.left').change(function () {
        if (!is_loading_data) {
            var me = $(this);
            ...
            if (typeof data === 'string') {
                ...
                switch (data.type) {
                    case 's':
                        newVal = prompt('Please, specify a constant value:', TrimChar(ctl.data('left'), '"'));
                        if (newVal != null) {
                            cancel = false;
                        ...
                        break;
                    case 'f':
                        Microsoft.Dynamics.NAV.InvokeExtensibilityMethod('EditFormula', [ctl.attr('id'), true, ctl.data('left')]);
                        break;
                    case 'v':
                        cancel = false;
                        newVal = '[' + data.n + ']';
            ...
            if (cancel)
                me.find('input').val(ctl.data('left'));
            else {
                me.find('input').val(newVal);
                ctl.data('left', newVal);
            }
        }
    });
    clone.find('.left input').blur(function () {
        var me = $(this);
        var ctl = me.closest('.condition-control');
        me.val(ctl.data('left'));
    });
    clone.find('.right').change(function () {
        if (!is_loading_data) {
            var me = $(this);
            var ctl = me.closest('.condition-control');
            var data = me.data('item');
            ...
            if (typeof data === 'string') {
                ...
                switch (data.type) {
                    case 's':
                        newVal = prompt('Please, specify a constant value:', TrimChar(ctl.data('right'), '"'));
                        if (newVal != null) {
                            cancel = false;
                            if (newVal == '' || isNaN(newVal))
                                newVal = '"' + newVal + '"';
                    ...
                    case 'r':
                        ...
                        break;
                    case 'c':
                        newVal = prompt('Please, specify pipe (\'|\') separated values:', TrimChar(ctl.data('right'), '(', ')'));
                        if (newVal != null) {
                            cancel = false;
                            newVal = '(' + newVal + ')';
                        }
                    ...
                    case 'v':
                        ...
                        break;
                    case 'f':
                        Microsoft.Dynamics.NAV.InvokeExtensibilityMethod('EditFormula', [ctl.attr('id'), false, ctl.data('right')]);
                        break;
                }
            }
            if (cancel) {
                me.find('input').val(ctl.data('right'));
            }
            else {
                me.find('input').val(newVal);
                ctl.data('right', newVal);
            }
        }
    });
    clone.find('.right input').blur(function () {
        var me = $(this);
        var ctl = me.closest('.condition-control');
        me.val(ctl.data('right'));
    });
    clone.find('input').change(function (e) {
        e.stopPropagation();
    });
    Microsoft.Dynamics.NAV.InvokeExtensibilityMethod('ItemAdded', []);
    return clone;
}
...
function AppendLine(leftHand, comparison, rightHand) {
    try {
        ...
        if (leftHand) {
            ...
            SetComboboxValue(cc.find('.left'), function (obj) {
                if (obj.type == 'v')
                    return obj.n == val;
                else
        ...
        cc.data('right', rightHand);
        SetComboboxValue(cc.find('.comparison'), comparison);
    }
    catch (e) {
        alert(e);
    }
...
function ApplyFormula(id, left, formula) {
    var ctl = $('#' + id);
    if (left) {
        ctl.find('.left input').val(formula);
        ctl.data('left', formula);
    }
    else {
        ctl.find('.right input').val(formula);
        ctl.data('right', formula);
    }
}
...
function ThrowConditionDataError(cd, e) {
    var err = 'Condition #' + (cd.length + 1) + ': ' + e;
    window.setTimeout(function () { Microsoft.Dynamics.NAV.InvokeExtensibilityMethod('ConditionDataError', [err]); }, 10);
    return false;
}
function PostConditionData(cd) {
    window.setTimeout(function () {
        ...
        for (var i = 0; i < cd.length; i++) {
            var c = cd[i];
            Microsoft.Dynamics.NAV.InvokeExtensibilityMethod('ReportLine', [c.leftHand, c.comparison, c.rightHand]);
        }
        Microsoft.Dynamics.NAV.InvokeExtensibilityMethod('LineReportComplete', []);
    }, 10);
}
