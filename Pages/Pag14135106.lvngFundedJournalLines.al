    ...
    layout
    {
        area(Content)
        {
            repeater(Group)
            {
                ...
                field(Blocked; Blocked) { Visible = false; ApplicationArea = All; }
                field("First Payment Due To Investor"; "First Payment Due To Investor") { Visible = false; ApplicationArea = All; }
                field("Date Sold"; "Date Sold") { Visible = false; ApplicationArea = All; }
            }
        }
        area(Factboxes)
        {
    ...
    actions
    {
        area(Processing)
        {
            action(Import)
            {
                ...
                begin
                    ...
                    if Page.RunModal(0, LoanImportSchema) = Action::LookupOk then begin
                        ...
                        Commit();
                        ValidateFundedJournal.ValidateFundedLines("Loan Journal Batch Code");
            ...
            action(CreateDocuments)
            {
                ...
                begin
                    CreateFundedDocuments.CreateDocuments("Loan Journal Batch Code");
                end;
            }
