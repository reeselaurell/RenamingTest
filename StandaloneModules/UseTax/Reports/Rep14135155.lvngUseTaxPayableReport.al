report 14135155 lvngUseTaxPayableReport
{
    ...
    dataset
    {
        ...
        {
            RequestFilterFields = "Pay-to Vendor No.", "Posting Date", lvngDeliveryState;
        ...
        dataitem(StateLoop; lvngState)
        {
            ...
            dataitem(OutputLoop; Integer)
            {
                ...
                var
                    CrLineCount: Integer;
                begin
                    PurchInvLine.SetCurrentKey(lvngDeliveryState);
                    PurchCrMemoLine.SetCurrentKey(lvngDeliveryState);
                    PurchInvLine.FilterGroup(5);
                    PurchInvLine.SetRange(lvngDeliveryState, StateLoop.Code);
                    PurchInvLine.SetFilter("No.", '<>%1', '');
                    PurchInvLine.SetRange(lvngUseSalesTax, true);
                    PurchInvLine.FilterGroup(0);
                    PurchCrMemoLine.FilterGroup(5);
                    PurchCrMemoLine.SetRange(lvngDeliveryState, StateLoop.Code);
                    PurchCrMemoLine.SetFilter("No.", '<>%1', '');
                    PurchCrMemoLine.SetRange(lvngUseSalesTax, true);
                ...
                begin
                    ...
                    if Number <= InvLineCount then begin
                        GetInvHeader();
                        DeliveryState := PurchInvLine.lvngDeliveryState;
                        ...
                        GetCrHeader();
                        DeliveryState := PurchCrMemoLine.lvngDeliveryState;
            ...
            area(Content)
            {
                group(Excel)
                {
                    ...
                    field(NumberFormat; NumberFormat)
                    {
                        ...
                        ApplicationArea = All;
                        TableRelation = lvngNumberFormat;
    ...
    var
        ...
        LineAmount: Decimal;
        GLAccount: Code[20];
        TaxFrequency: Enum lvngTaxFrequency;
        TaxRate: Decimal;
        TaxAmount: Decimal;
