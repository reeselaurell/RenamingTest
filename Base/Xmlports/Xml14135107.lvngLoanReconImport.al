xmlport 14135107 lvngLoanReconImport
{
    Direction = Import;
    Format = VariableText;
    FieldSeparator = '<TAB>';
    ...
    schema
    {
        textelement(Root)
        {
            tableelement(LoanReconBuffer; lvngLoanReconciliationBuffer)
            {
    ...
    procedure GetData(var LoanReconciliationBuffer: Record lvngLoanReconciliationBuffer)
    var
    ...
    begin
        if GLAccountSource = GLAccountSource::File then begin
            ...
            repeat
                ...
                GLEntry.Reset();
                GLEntry.SetCurrentKey("G/L Account No.", "Reason Code", lvngLoanNo);
                GLEntry.CopyFilters(GLEntry);
                GLEntry.SetRange(lvngLoanNo, LoanReconciliationBuffer."Loan No.");
            ...
            LoanReconBuffer.FindSet();
            repeat
                GLEntry.SetRange(lvngLoanNo, LoanReconBuffer."Loan No.");
                if GLEntry.FindSet() then
                    repeat
