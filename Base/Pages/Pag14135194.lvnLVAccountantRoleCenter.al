page 14135194 "lvnLVAccountantRoleCenter"
{
    ...
    layout
    {
        area(RoleCenter)
        {
            part(Headline; lvnLVAccountantHeadline) { ApplicationArea = Basic, Suite; }
            part(GeneralActivities; lvnLVAccountantFinanceAct) { ApplicationArea = Basic, Suite; }
            part(LoanActivitites; lvnLVAccountantLoanActivities) { ApplicationArea = Basic, Suite; }
            part(CloseManagerActivities; lvnCloseManagerActivities) { ApplicationArea = Basic, Suite; }
            group(PerformanceData)
            {
                part(TopBranch; lvnLVAcctBranchTopPerfPart) { ApplicationArea = Basic, Suite; }
                part(TopLO; lvnLVAcctLOTopPerfPart) { ApplicationArea = Basic, Suite; }
                part(BotBranch; lvnLVAcctBranchBotPerfPart) { ApplicationArea = Basic, Suite; }
                part(BotLO; lvnLVAcctLOBotPerfPart) { ApplicationArea = Basic, Suite; }
            }
        }
    }
    ...
    actions
    {
        ...
        area(Sections)
        {
            ...
            group(CashManagement)
            {
                ...
                action(QuickPay) { Caption = 'Quick Pay'; ApplicationArea = All; RunObject = page lvnQuickPayWorksheet; }
            ...
            group(LoanManagement)
            {
                ...
                action(LoanList) { Caption = 'Loan List'; ApplicationArea = All; RunObject = page lvnLoanList; }
                action(LoanJournals) { Caption = 'Loan Journals'; ApplicationArea = All; RunObject = page lvnLoanJournalBatches; }
                action(ReverseSalesDocument) { Caption = 'Reverse Sales Document'; ApplicationArea = All; RunObject = page lvnReverseSalesDocuments; }
                action(ReversePurchaseDocuments) { Caption = 'Reverse Purchase Documents'; ApplicationArea = All; RunObject = page lvnReversePurchaseDocuments; }
                action(QuickTrace) { Caption = 'Quick Trace'; ApplicationArea = All; RunObject = page lvnQuickTrace; }
                action(LoanFileReconciliationWorksheet) { Caption = 'Loan File Reconciliation Worksheet'; ApplicationArea = All; RunObject = page lvnLoanFileReconWorksheet; }
            }
            ...
            group(Reports)
            {
                group(FinancialReports)
                {
                    ...
                    action(LoanLevelValuesFinancial) { Caption = 'Loan Level Worksheet'; ApplicationArea = All; RunObject = report lvnLoanLevelWorksheet; }
                    ...
                    action(ReportGeneratorBatchList) { Caption = 'Report Generator Batch List'; ApplicationArea = All; RunObject = page lvnReportGeneratorBatchList; }
                }
                ...
                group(LoanLevelReports)
                {
                    ...
                    action(GLReconcilitaionPage) { Caption = 'G/L Reconciliation'; ApplicationArea = All; RunObject = page lvnGLReconciliation; }
                    action(GLEntriesByLoanNo) { Caption = 'G/L Entries by Loan No.'; ApplicationArea = All; RunObject = report lvnGLEntriesByLoanVer2; }
                    action(AccountRecByLoan) { Caption = 'Account Rec. by Loan'; ApplicationArea = All; RunObject = report lvnAccountReconByLoan; }
                    action(GLReconciliationReport) { Caption = 'G/L Reconciliation Printout'; ApplicationArea = All; RunObject = report lvnGeneralLedgerRecSimplified; }
                    action(GLByReasonCode) { Caption = 'General Ledger by Reason Code'; ApplicationArea = All; RunObject = report lvnGeneralLedgerByReasonCode; }
                    action(AverageDailyTrialBalance) { Caption = 'Average Daily Trial Balance'; ApplicationArea = All; RunObject = report lvnAverageDailyTrialBalance; }
                    action(LoanProfitability) { Caption = 'Loan Profitability'; ApplicationArea = All; RunObject = report lvnLoanProfitability; }
                    action(LoanFileReconciliationWorksheetAction) { Caption = 'Loan File Reconciliation Worksheet'; ApplicationArea = All; RunObject = page lvnLoanFileReconWorksheet; }
                    action(LoanFeesReport) { Caption = 'Loan Fees Report'; ApplicationArea = All; RunObject = report lvnLoanFeesReport; }
                    action(LoanLevelValuesLoanLevel) { Caption = 'Loan Level Worksheet'; ApplicationArea = All; RunObject = report lvnLoanLevelWorksheet; }
                }
                ...
                group(PayablesReports)
                {
                    ...
                    action(UseTaxPayable) { Caption = 'Use Tax Payable'; ApplicationArea = All; RunObject = report lvnUseTaxPayableReport; }
                    action(LoanLevelValuesPayables) { Caption = 'Loan Level Worksheet'; ApplicationArea = All; RunObject = report lvnLoanLevelWorksheet; }
                    ...
                    action(VendorCheckApplication) { Caption = 'Vendor Check Application Report'; ApplicationArea = All; RunObject = report lvnVendCheckApplicationReport; }
                }
            }
        }
    }
