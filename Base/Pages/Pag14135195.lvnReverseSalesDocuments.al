page 14135195 "lvnReverseSalesDocuments"
{
                    ...
                    if Rec.FindSet() then
                        repeat
                            ...
                            if Rec."Document Type" = Rec."Document Type"::"Credit Memo" then begin
                                ...
                                LoanNoLength := StrLen(SalesCrMemoHeader.lvnLoanNo);
                                LastSymbolOfLoanNo := CopyStr(SalesCrMemoHeader.lvnLoanNo, LoanNoLength, 1);
                                if LastSymbolOfLoanNo = 'X' then
                                    if not Confirm(ReverseConfirmMsg, false, Rec."No.") then
                                        Error(CancelledErr);
                                SalesCrMemoHeader.lvnLoanNo := SalesCrMemoHeader.lvnLoanNo + 'X';
                                SalesCrMemoHeader.Modify();
                                SalesCrMemoLine.Reset();
                                SalesCrMemoLine.SetRange("Document No.", SalesCrMemoHeader."No.");
                                if SalesCrMemoLine.FindSet(true) then
                                    repeat
                                        if SalesCrMemoLine.lvnLoanNo <> '' then begin
                                            SalesCrMemoLine.lvnLoanNo := SalesCrMemoLine.lvnLoanNo + 'X';
                                ...
                                if GLEntry.FindSet(true) then
                                    repeat
                                        if GLEntry.lvnLoanNo <> '' then begin
                                            GLEntry.lvnLoanNo := GLEntry.lvnLoanNo + 'X';
                                            GLEntry.Modify();
                                        end;
                                ...
                                if CustLedgEntry.FindSet(true) then
                                    repeat
                                        CustLedgEntry.lvnLoanNo := Rec.lvnLoanNo + 'X';
                            ...
                            if Rec."Document Type" = Rec."Document Type"::Invoice then begin
                                ...
                                LastSymbolOfLoanNo := CopyStr(SalesInvHeader.lvnLoanNo, LoanNoLength, 1);
                                if LastSymbolOfLoanNo = 'X' then
                                    if not Confirm(ReverseConfirmMsg, false, Rec."No.") then
                                        Error(CancelledErr);
                                SalesInvHeader.lvnLoanNo := SalesInvHeader.lvnLoanNo + 'X';
                                SalesInvHeader.Modify();
                                SalesInvLine.Reset();
                                SalesInvLine.SetRange("Document No.", SalesInvHeader."No.");
                                if SalesInvLine.FindSet(true) then
                                    repeat
                                        if SalesInvLine.lvnLoanNo <> '' then begin
                                            SalesInvLine.lvnLoanNo := SalesInvLine.lvnLoanNo + 'X';
                                            SalesInvLine.Modify();
                                        end;
                            ...
                            SalesHeader.lvnLoanNo := SalesHeader.lvnLoanNo + 'X';
                            SalesHeader.Modify();
                            SalesLine.Reset();
                            ...
                            if SalesLine.FindSet(true) then
                                repeat
                                    if SalesLine.lvnLoanNo <> '' then begin
                                        SalesLine.lvnLoanNo := SalesLine.lvnLoanNo + 'X';
                                        SalesLine.Modify();
                                    end;
